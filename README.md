...
* **3D world** – The game uses Three.js to render a sky, ground, mountains and
  an aeroplane model built from basic shapes.  In this enhanced edition the
  sky, ground and mountains are textured with high‑resolution images to
  increase realism.  Lighting and fog add atmosphere and depth to the scene.
...
* **Responsive design** – The canvas resizes to fit the user’s device and
  supports high‑DPI displays.  On‑screen directional buttons overlay the
  display on touch devices so that mobile users can steer the plane without a
  keyboard.
